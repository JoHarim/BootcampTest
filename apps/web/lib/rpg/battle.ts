...
// 계약: types.ts / 수치·공식: docs/plan/rpg-design.md 4장 + 9장(십성 시너지) + 10장(신살) + 11장(운의 흐름)
import type {
  BattleCommand,
  BattleEvent,
  BattleState,
...
import { BRANCH_POOL, SINSALS, STEM_POOL, SYNERGIES } from "./content";
...
const MIN_COOLDOWN = 1; // 화개 감산 후 스킬 쿨다운 하한 (설계서 10장)
...
// 전투 시작 상태 구성. 레벨 성장 → 직업 패시브 → 운의 흐름 순서로 곱연산 적용.
export function initBattle(
  ...
  for (const f of flows) {
    // 같은 스탯에 겹치면 곱연산 스택 — 대운·세운·월운·일운 (설계서 11장)
  ...
  // 화개 신살 — 스킬 쿨다운 감소 (설계서 10장, 하한 1턴)
  const cooldownCut = c.sinsals.includes("화개")
    ? (SINSALS.화개.cooldownCut ?? 0)
    : 0;
  ...
  return {
    ...
    job: c.job,
    skillCooldown: 0,
    skillCooldownMax: Math.max(MIN_COOLDOWN, c.job.skill.cooldown - cooldownCut),
    flows: [...flows], // 표시용 보관 (스탯 반영은 위에서 완료)
...
export function stepBattle(
  ...
  let foeHp = s.foe.hp;
  let reviveUsed = s.reviveUsed;
  let over = false;
  let won = false;

  // ── 역마 신살 — 첫 스텝, 본 행동 전 선제 일격 1회 (설계서 10장) ──
  // 변동·치명 없음 = 시드 미소비 (기존 시드 순서 보존)
  if (s.turn === 1 && s.sinsals.includes("역마")) {
    const mult = elementMultiplier(s.player.element, s.foe.element);
    const dmg = Math.max(
      MIN_DAMAGE,
      Math.round(
        s.player.atk * (SINSALS.역마.strikePower ?? 0) * mult -
          s.foe.def * DEF_REDUCE,
      ),
    );
    foeHp = Math.max(0, foeHp - dmg);
    events.push({
      kind: "sinsal",
      text: `🐎 역마살 — 선제 일격! ${dmg} 피해`,
      amount: dmg,
    });
    if (foeHp === 0) {
      // 선제 일격만으로 격파 — 본 행동·반격 생략, 즉시 승리
      over = true;
      won = true;
  ...
  if (!over) {
    ...
    if (useSkill) {
      ...
      cooldown = s.skillCooldownMax; // 사용 직후 쿨다운 재설정 (화개 감산 반영값)
    } else {
    ...
    if (foeHp === 0) {
      ...
      if (playerHp === 0) {
        if (!reviveUsed && s.sinsals.includes("천을귀인")) {
          // ── 천을귀인 신살 — 전투당 1회, 치명상에서 부활 (설계서 10장) ──
          playerHp = Math.round(
            (s.player.maxHp * (SINSALS.천을귀인.reviveHpPct ?? 0)) / 100,
          );
          reviveUsed = true;
          events.push({
            kind: "sinsal",
            text: `🌟 천을귀인 — 하늘이 붙잡았다! HP ${playerHp}`,
            amount: playerHp,
          });
        } else {
          over = true;
...
export function rollLetterDrop(
  ...
  for (let i = 0; i < count; i += 1) {
    const drawn = drawLetter(s, elementPool, allPool);
    s = drawn.seed;
    letters.push(drawn.letter);
  }
  // 도화 보너스 롤 — 0%면 시드 미소비 (기존 호출 재현성 보존)
  if (dropBonusPct > 0) {
    const bonusRoll = roll(s);
    s = bonusRoll.seed;
    if (bonusRoll.value < dropBonusPct / 100) {
      const drawn = drawLetter(s, elementPool, allPool);
      s = drawn.seed;
      letters.push(drawn.letter);
    }
